...
class Target {
  ...
  draw(){
    const size=this.radius*2.8; let img=null;
...
class Farmer {
  ...
  update(){
    ...
    this.t++; this.phaseTimer++; const bob=Math.sin(this.t*0.25)*2;
    if (this.phase==='enter'){ this.x+=this.enterSpeedX*1.1; this.y=180+bob;
      if (this.x>=this.poseX){ this.x=this.poseX; this.phase='pose'; this.phaseTimer=0; } }
    else if (this.phase==='pose'){ this.y=this.poseY+Math.sin(this.t*0.15)*1.0;
  ...
  draw(){ if (IMGS && IMGS.farmerAngry) ctx.drawImage(IMGS.farmerAngry,this.x,this.y,this.width,this.height);
...
function bumpCombo(px,py,show=true){ const now=performance.now(); comboCount = (now-lastHitTime<=COMBO_TIMEOUT)? (comboCount+1):1; lastHitTime=now;
...
function update(){
  ...
  if (!isPaused){
    if (inBonusRound && bonusFarmer) bonusFarmer.update();
      ...
      for (let i=0;i<targets.length;i++){ const t=targets[i];
...
function startGame(){
  ...
  exitBtn.disabled = true;                // disable exit during countdown
  showCountdown(()=>{
    beginMainTimer();
...
function showCountdown(cb){
  const msgs=['Ready...','Set...','Go!']; let i=0;
  countdownActive=true; countdownDiv.style.display='block'; countdownDiv.innerText=msgs[i];
  countdownTimerId = setInterval(()=>{
    i++;
    if (i>=msgs.length){ clearInterval(countdownTimerId); countdownTimerId=null; countdownActive=false; countdownDiv.style.display='none'; cb(); }
    else countdownDiv.innerText=msgs[i];
  }, 1000);
}
function triggerBonusRound(){
  inBonusRound=true; targets=[]; bonusFarmer=new Farmer();
  showBonusPrompt=true; setTimeout(()=>{ showBonusPrompt=false; bonusFarmer.active=true; }, 1000);
}
...
canvas.addEventListener('click',(e)=>{
  ...
  tomatoes.push(new Tomato(x,y)); cooldown=true; setTimeout(()=>cooldown=false,600);
});
...
/* Allow ESC to close the topmost panel */
window.addEventListener('keydown',(e)=>{
  if (e.key==='Escape'){
    if (updateLogPanel.style.display==='flex') closePanel(updateLogPanel);
    else if (settingsPanel.style.display==='flex') closePanel(settingsPanel);
  }
});
...
function setScreen(screen){
  mainMenu.classList.remove('active'); highscoresScreen.classList.remove('active');
  nameEntry.classList.remove('active'); toMenuOverlay.classList.remove('active');
...
function highscoreDestinations(sc){
  const today=loadTodayScores().sort((a,b)=>b.score-a.score); const qualifiesToday = today.length<10 || sc>(today[9]?.score ?? -Infinity);
